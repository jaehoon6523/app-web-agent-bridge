      ...
      if (message.payload?.refreshCompleted) {
        ...
        try {
          ...
          if (!completed || completed.turnId !== saved.currentDeliveryId || !saved.lastBoundRunId?.startsWith("prep_")) {
            throw new ExtensionOperationError("RESPONSE_RECHECK_UNAVAILABLE", "?§Ïãú ?ïÏù∏??Ï§ÄÎπ??ëÎãµ???ÜÏäµ?àÎã§.");
          }
          ...
          if (!result?.ok) throw new ExtensionOperationError(result?.code ?? "RESPONSE_RECHECK_FAILED", result?.error ?? "?ëÎãµ ?¨Ìôï?∏Ïóê ?§Ìå®?àÏäµ?àÎã§.");
          const current = await store.read();
          if (current.currentDeliveryId !== completed.turnId || current.lastBoundSessionId !== saved.lastBoundSessionId) {
            throw new ExtensionOperationError("DELIVERY_RECOVERY_MISMATCH", "?¨Ìôï??Ï§??ÑÏÜ° ?Ä?ÅÏù¥ Î≥ÄÍ≤ΩÎêê?µÎãà??");
          }
    ...
    case "web.delivery.focus":
      try {
        ...
        if (state.tabId === null || state.lastBoundSessionId !== message.payload?.sessionId
          ...
          throw new ExtensionOperationError("DELIVERY_RECOVERY_MISMATCH", "?¥Ï†Ñ ?Ä????ù¥ Î≥ÄÍ≤ΩÎêê?µÎãà?? ?ÅÌÉúÎ•??§Ïãú ?ïÏù∏?òÏÑ∏??");
        }
        const tab = await chrome.tabs.get(state.tabId);
        if (canonicalChatGptUrl(tab.url) !== state.conversationUrl) throw new ExtensionOperationError("DELIVERY_RECOVERY_MISMATCH", "Í∏∞Ï°¥ ??ù¥ ?§Î•∏ ?Ä?îÎ°ú ?¥Îèô?àÏäµ?àÎã§.");
        await focusTab(tab);
...
async function handleDeliveryDiscard(message) {
  try { turnGate.assertIdle("Delivery discard"); const state = await store.read(), expected = message.payload || {};
    if (state.currentDeliveryId !== expected.currentDeliveryId || state.lastBoundSessionId !== expected.sessionId
      || state.lastBoundRunId !== expected.runId || state.conversationUrl !== expected.conversationUrl) throw new ExtensionOperationError("DELIVERY_RECOVERY_MISMATCH", "?êÍ∏∞ ?Ä???ÑÏÜ° identityÍ∞Ä ?ÑÏû¨ Í∏∞Î°ùÍ≥??§Î¶Ö?àÎã§.");
    if (expected.unresolvedResultConfirmed !== true || expected.noAutomaticResendConfirmed !== true || typeof expected.reason !== "string" || expected.reason.trim().length < 3) throw new ExtensionOperationError("DISCARD_CONFIRMATION_REQUIRED", "ÎØ∏Ìôï??Í≤∞Í≥º?Ä ?êÎèô ?¨Ï†Ñ??Í∏àÏ?Î•??ïÏù∏?òÍ≥† ?êÍ∏∞ ?¨Ïú†Î•??ÖÎ†•?òÏÑ∏??");
...
async function handleDeliveryStop(message) {
  try {
    ...
    if (!state.currentDeliveryId || expected.currentDeliveryId !== state.currentDeliveryId
      ...
      throw new ExtensionOperationError("DELIVERY_RECOVERY_MISMATCH", "Ï¢ÖÎ£å ?Ä???ÑÏÜ°??Î≥ÄÍ≤ΩÎêê?µÎãà?? ?ÅÌÉúÎ•??§Ïãú ?ïÏù∏?òÏÑ∏??");
    }
    ...
    if (details.activeRequestId !== state.currentDeliveryId || details.observedConversationUrl !== state.conversationUrl) {
      throw new ExtensionOperationError("DELIVERY_STOP_UNAVAILABLE", "?¥Îãπ ?ÑÏÜ°???ùÏÑ± ?ëÏóÖ???ùÎ≥Ñ?????ÜÏäµ?àÎã§. ?Ä????óê???ùÏÑ± ?ÅÌÉúÎ•??ïÏù∏?òÏÑ∏??", details);
    }
    const stopped = await chrome.tabs.sendMessage(state.tabId, { type: "agent.cancel", requestId: state.currentDeliveryId });
    if (!stopped?.ok || !stopped.cancelled) throw new ExtensionOperationError("INTERRUPT_NOT_CONFIRMED", "?ùÏÑ± Ï¢ÖÎ£å ?îÏ≤≠???ïÏù∏?òÏ? ?äÏïò?µÎãà??", details);
    ...
    do {
      ...
      if (details.currentDeliveryId !== state.currentDeliveryId || details.sessionId !== state.lastBoundSessionId) {
        throw new ExtensionOperationError("DELIVERY_RECOVERY_MISMATCH", "Ï¢ÖÎ£å ?ïÏù∏ Ï§??ÑÏÜ° ?Ä?ÅÏù¥ Î≥ÄÍ≤ΩÎêê?µÎãà??", details);
      }
    ...
    throw new ExtensionOperationError("INTERRUPT_NOT_CONFIRMED", "Ï¢ÖÎ£åÎ•??îÏ≤≠?àÏ?Îß??ùÏÑ± Ï¢ÖÎ£å???ÑÏßÅ ?ïÏù∏?òÏ? ?äÏïò?µÎãà?? ?ÅÌÉúÎ•??§Ïãú ?ïÏù∏?òÏÑ∏??", details);
  } catch (error) {
    send({ type: "web.session.error", requestId: message.requestId, payload: errorPayload(error) });
  }
}
async function handleDeliveryRecovery(message) {
  ...
  try {
    ...
    if (!state.currentDeliveryId || expected.currentDeliveryId !== state.currentDeliveryId
      || expected.runId !== state.lastBoundRunId || expected.sessionId !== state.lastBoundSessionId
      || expected.conversationUrl !== state.conversationUrl) {
      throw new ExtensionOperationError("DELIVERY_RECOVERY_MISMATCH", "Î≥µÍµ¨ ?Ä???ÑÏÜ°??Î≥ÄÍ≤ΩÎêê?µÎãà?? Ï§ÄÎπÑÎ? ?§Ïãú ?îÏ≤≠?òÏÑ∏??");
    }
    ...
    if (matched.status !== "BOUND") {
      throw new ExtensionOperationError("DELIVERY_RECOVERY_UNCONFIRMED", "?¥Ï†Ñ ?ÑÏÜ°??Î≥µÍµ¨???Ä????ùÑ ?ïÏ†ï?????ÜÏäµ?àÎã§.", {
    ...
    if (!details.pageReachable || details.pageBusy !== false || details.generating !== false
      ...
      throw new ExtensionOperationError("DELIVERY_RECOVERY_UNCONFIRMED", "?¥Ï†Ñ ?Ä?îÏùò ?ùÏÑ± Ï¢ÖÎ£åÎ•??ïÏù∏?òÏ? Î™ªÌñà?µÎãà?? ?¥Îãπ ?Ä????ùÑ ?ïÏù∏?òÏÑ∏??", details);
    }
...
async function prepareBoundSession(payload) {
  ...
  if (requested.bootstrap) {
    ...
    if (!page?.ready || page.busy || page.generating || page.url !== "https://chatgpt.com/") {
      throw new ExtensionOperationError("ROOT_NOT_READY", "ChatGPT ???Ä???ÖÎ†•Ï∞ΩÏùÑ ?¨Ïö©?????ÜÏäµ?àÎã§.", page);
    }
...
async function handlePrompt(message) {
  ...
  try {
    ...
    if (bootstrap && canonicalChatGptUrl(tab.url) !== "https://chatgpt.com/") throw new ExtensionOperationError("ROOT_CHANGED", "?†ÌÉù?????Ä????ùò Ï£ºÏÜåÍ∞Ä Î≥ÄÍ≤ΩÎêò?àÏäµ?àÎã§.");
    ...
    if (bootstrap) {
      ...
      if (current.currentDeliveryId !== message.requestId || current.lastBoundSessionId !== reservedState.lastBoundSessionId
        ...
        throw new ExtensionOperationError("TURN_BINDING_CHANGED", "???Ä???ùÏÑ± Í≤∞Í≥º?Ä ?ÑÏÜ°????ùò ?ùÎ≥Ñ?êÍ? ?ºÏπò?òÏ? ?äÏäµ?àÎã§.");
      }
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === "bridge.clearLegacyTestDelivery") {
    void (async () => {
      ...
      if (!isLegacy) throw new ExtensionOperationError("NO_LEGACY_DELIVERY", "??†ú???§Îûò??Î∏åÎ¶øÏßÄ ?ÑÏÜ°???ÜÏäµ?àÎã§.");
