...
export class DashboardController {
  ...
  async snapshot(runId = null) {
    ...
    if (codeRun) {
      ...
      snapshot.runs = runs.map(({ runId, objective, phase, projectRef, targetRoot, createdAt, updatedAt }) =>
        ({ runId, objective, phase, targetRoot:projectRef?.targetRoot ?? targetRoot ?? null, createdAt, updatedAt }));
      return snapshot;
    }
    ...
    return {
      run,
      runs: runs.map(({ runId: id, objective, phase, projectRef, targetRoot, createdAt, updatedAt }) =>
        ({ runId: id, objective, phase, targetRoot:projectRef?.targetRoot ?? targetRoot ?? null, createdAt, updatedAt })),
