...
RESULT에는 `AWAITING_APPLY`, `APPLIED`, `COMPLETE`, `CANCELLED`, `INCONCLUSIVE`,
`FAILED`가 포함된다. 실행의 `HOLD`, `RECOVERY_REQUIRED`는 WORK에 남아 복구·재시도
행동을 표시한다.
...
```text
START: START_IDLE | VALIDATING | CONNECTING_WEB | WAITING_WEB_RESPONSE
       | RECOVERY_REQUIRED | WEB_BLOCKED | FAILED
...
WORK: RUN_CREATED | PROVISIONING | WORKER_RUNNING | VERIFYING
      | REVIEW_RUNNING | REWORK | APPLYING | STOPPING | HOLD | RECOVERY_REQUIRED
RESULT: AWAITING_APPLY | APPLIED | COMPLETE | CANCELLED | INCONCLUSIVE | FAILED
```
